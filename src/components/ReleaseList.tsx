...
export const ReleaseList = ({ releases, manifest, userPlatform }: ReleaseListProps) => {
  ...
  return (
    <div className={c(['space-y-6'])}>
      {/* Filters */}
      <div className={c(['bg-white', 'rounded-lg', 'shadow', 'p-6'])}>
        <div className={c(['flex', 'items-center', 'mb-4'])}>
          <FaFilter className={c(['text-gray-500', 'mr-2'])} />
          <h3 className={c(['text-lg', 'font-medium', 'text-gray-900'])}>Filters</h3>
        </div>
      ...
      {filteredReleases.map((release) => (
        <div key={release.id} className="bg-white rounded-lg shadow">
          {/* Release Header */}
          <div className="p-6 border-b border-gray-200">
            <div className="flex items-center justify-between">
              <div className="flex items-center">
                ...
                <div>
                  <h2 className="text-xl font-semibold text-gray-900">{release.name}</h2>
              ...
              {release.assets.map((asset) => (
                ...
                >
                  <div className="flex-1">
                    <div className="flex items-center">
                      <h4 className="font-medium text-gray-900 mr-3">{asset.name}</h4>
                      {asset.isRecommended && (
                          ...
                          {asset.matchedRule.tags && asset.matchedRule.tags.length > 0 && (
                            <span className="flex items-center">
                              {asset.matchedRule.tags.map((tag) => (
                                <span
                                  key={tag}
                                  className="px-2 py-1 bg-gray-100 text-gray-700 text-xs rounded mr-1"
                                >
                                  {tag}
                                </span>
                              ))}
                    ...
                    {asset.matchedRule?.description && (
                      <p className="text-sm text-gray-600 mt-1">{asset.matchedRule.description}</p>
                    )}
                  </div>
      ...
      {filteredReleases.length === 0 && (
        <div className="text-center py-12 bg-white rounded-lg shadow">
          <FaDownload className="h-16 w-16 text-gray-400 mx-auto mb-4" />
